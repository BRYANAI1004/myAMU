...
import type { LoginEmailStatus } from '../lib/api'

export type LoginEmailPanelApi = {
  fetchStatus: (options?: { signal?: AbortSignal }) => Promise<LoginEmailStatus>
  sendCode: (email: string) => Promise<void>
  verifyCode: (email: string, code: string) => Promise<LoginEmailStatus>
}

type Props = {
  ready: boolean
  embedded?: boolean
  api: LoginEmailPanelApi
  heading?: string
  intro: string
...
export function LoginEmailPanel({
  ...
  intro,
  inputIdPrefix,
  verifySuccessMessage = 'Email verified — you can sign in with a one-time code.',
}: Props) {
  const [status, setStatus] = useState<LoginEmailStatus | null>(null)
  ...
  async function handleVerify() {
    ...
    setActionSuccess(null)
    try {
      const next = await api.verifyCode(email, code)
      setStatus(next)
      resetEditState()
      setActionSuccess(verifySuccessMessage)
    } catch (e) {
  ...
  const headingId = `${inputIdPrefix}-heading`

  return (
    <Wrapper className={wrapperClass} aria-labelledby={headingId}>
      <h3
        id={headingId}
        className={embedded ? 'portal-profile-section__title' : 'portal-section-heading'}
      >
        {heading}
      </h3>
      <p className="portal-login-email-intro">{intro}</p>
      ...
      {!loading && !loadError && (!status?.verified || editing) ? (
        <div className="portal-login-email-form">
          <label className="portal-login-email-field" htmlFor={`${inputIdPrefix}-input`}>
            <span className="portal-login-email-label">Email</span>
            <input
          ...
          {codeSent ? (
            <label className="portal-login-email-field" htmlFor={`${inputIdPrefix}-code`}>
