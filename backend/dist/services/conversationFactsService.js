...
export function sanitizeConversationFacts(raw) {
    if (raw == null || typeof raw !== "object")
        return undefined;
    const rec = raw;
    const statedName = sanitizeShortText(rec.statedName, MAX_NAME_LENGTH);
    const preferredLanguage = rec.preferredLanguage === "en" || rec.preferredLanguage === "zh"
        ? rec.preferredLanguage
        : undefined;
    if (statedName == null && preferredLanguage == null)
        return undefined;
    return {
...
export function buildSafeLoggedInUserContext(studentId, profile) {
...
export function formatIdentityContextBlock(identityContext) {
...
function detectSelfReferentialQuestionKind(question) {
    const trimmed = question.trim();
    const lower = trimmed.toLowerCase();
    if (/\b(what('?s| is)\s+my\s+name|do\s+you\s+remember\s+my\s+name|who\s+am\s+i)\b/i.test(lower) ||
        /我叫什么|你还记得我的名字吗|你记得我叫什么|我的名字是什么|我是谁/.test(trimmed)) {
        return "name";
    }
    if (/\b(do\s+you\s+know\s+my\s+gender|am\s+i\s+(male|female)|what\s+gender\s+am\s+i)\b/i.test(lower) ||
        /我是男是女|你知道我的性别吗|我的性别是什么/.test(trimmed)) {
        return "gender";
    }
    if (/\b(what('?s| is)\s+my\s+student\s+id|do\s+you\s+know\s+my\s+student\s+id)\b/i.test(lower) ||
        /我的学号是什么|你知道我的学号吗/.test(trimmed)) {
        return "student_id";
    }
    if (/\b(what\s+program\s+am\s+i\s+in|what('?s| is)\s+my\s+program|do\s+you\s+know\s+my\s+program)\b/i.test(lower) ||
        /我是什么项目|我读什么项目|我的项目是什么|你知道我的项目吗/.test(trimmed)) {
        return "program";
    }
    return null;
}
export function answerSelfReferentialQuestion(question, identityContext) {
...
//# sourceMappingURL=conversationFactsService.js.map