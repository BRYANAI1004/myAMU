...
function hasPersonalRecordCue(value) {
    return /\b(i|me|my|mine|am i|do i|did i|have i|for me)\b/i.test(value);
}
function hasPolicyCue(value) {
    return /\b(policy|handbook|catalog|rule|rules|requirement|requirements|deadline|deadlines|prerequisite|prerequisites|graduation|attendance|probation|refund|withdrawal policy|add\/drop|registration policy)\b/i.test(value);
}
function hasMixedApplicabilityCue(value) {
    return /\b(apply to me|apply in my case|how does it apply to me|based on my current record|based on my record|what should i pay attention to|for my situation|given my record)\b/i.test(value);
}
