...
function buildAvailableTermsFromRows(rows) {
    const byKey = new Map();
    for (const r of rows) {
...
function normalizeEnglishTitle(code, rawTitle, lookup) {
    const key = code.trim();
    if (key === "")
...
function rowFromMarks(r, lookup) {
    return {
        courseCode: r.code,
...
function rowFromClinic(r, lookup) {
    return {
        courseCode: r.code,
        courseTitle: normalizeEnglishTitle(r.code, r.course_title, lookup),
        term: r.term,
