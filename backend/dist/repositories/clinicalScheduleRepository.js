...
export async function listStudentClinicalAssignments(studentId) {
    ...
    const [rows] = await pool.query(`SELECT id, student_id, course_code, session_date, session_name, site, faculty, status, created_at
     FROM clinical_assignments
     WHERE TRIM(student_id) = TRIM(?)
     ORDER BY session_date ASC, id ASC`, [sid]);
