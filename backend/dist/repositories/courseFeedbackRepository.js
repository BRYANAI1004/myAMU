...
export async function listCourseFeedbackForCourseTermYear(pool, args) {
    const code = args.courseCode.trim();
    const term = args.term.trim();
    const year = Math.trunc(args.year);
    if (code === "" || term === "" || !Number.isFinite(year))
        return [];
    const [rows] = await pool.query(`SELECT q1_rating,
            q2_rating,
            q3_rating,
            q4_rating,
            q5_rating,
     ...
     WHERE course_code = ?
       AND term = ?
       AND year = ?
     ORDER BY submitted_at ASC, id ASC`, [code, term, year]);
    return rows.map((r) => {
        const commentRaw = r.comment;
        const submittedRaw = r.submitted_at;
        const submittedAt = submittedRaw == null
            ? null
        ...
        return {
            ...
            submitted_at: submittedAt != null && Number.isNaN(submittedAt.getTime())
                ? null
                : submittedAt,
        };
    });
